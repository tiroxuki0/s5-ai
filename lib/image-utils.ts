export function isValidImageUrl(url: string | undefined): boolean {
  ...
  const problematicPatterns = ["lookaside.instagram.com", "google_widget/crawler", "/seo/"]

  if (problematicPatterns.some((pattern) => url.includes(pattern))) {
    return false
  }

  // For external image search results, trust the imageUrl they provide
  // These are already validated image URLs from search results
  try {
